...
const newContent = `import { VercelRequest, VercelResponse } from '@vercel/node';
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    const dbSongId = \`\${artist}-\${title}\`.toLowerCase().replace(/[^a-z0-9]/g, '-');
    const firestoreUrl = \`https://firestore.googleapis.com/v1/projects/fanra-dev/databases/(default)/documents/song_details/\${dbSongId}\`;
    
    // Check if it already exists in cache
    try {
      const cacheSnap = await fetch(firestoreUrl);
      if (cacheSnap.ok) {
        const cacheData = await cacheSnap.json();
        if (cacheData?.fields) {
          const cachedDesc = cacheData.fields.description?.stringValue || "";
          const cachedLyrics = cacheData.fields.lyrics?.stringValue || "";
          if (cachedDesc && cachedDesc !== "Sorry, I couldn't find specific details for this song in the database.") {
            return res.status(200).json({ description: cachedDesc, lyrics: cachedLyrics });
          }
        }
      }
    } catch(e) {
    ...
    // Attempt to write to cache using the client's auth header
    ...
    if (authHeader) {
      try {
        ...
        await fetch(\`\${firestoreUrl}?updateMask.fieldPaths=title&updateMask.fieldPaths=artist&updateMask.fieldPaths=description&updateMask.fieldPaths=lyrics&updateMask.fieldPaths=updatedAt\`, {
          method: 'PATCH',
          headers: {
...
fs.writeFileSync('api/song-detail.ts', newContent);
