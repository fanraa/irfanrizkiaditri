...
const newFunc = `const fetchSongDetailForTrack = async (trackId: string | null, forceRegenerate: boolean = false) => {
    if (!trackId) return;
...
const regex = /const fetchSongDetailForTrack = async \(trackId: string \| null, forceRegenerate: boolean = false\) => \{[\s\S]*?  const removeFromQueue =/m;
