// reviews.js
// Menangani ulasan (rating + teks): simpan, render, sinkronisasi dengan Supabase.
...
async function fbGetReviews() {
  ...
  const { data, error } = await sb.from('reviews').select('*').order('ts', { ascending: false });
  if (error) throw error;
  return data;
}
async function saveReviewFirebase(review) {
  if (!window.sb) throw new Error('Supabase not available');
  const { data, error } = await sb.from('reviews').insert([review]);
  if (error) throw error;
  return data;
}
async function fbDeleteReview(id) {
  if (!window.sb) throw new Error('Supabase not available');
  const { data, error } = await sb.from('reviews').delete().eq('id', id);
  if (error) throw error;
  return data;
}
...
async function renderReviews() {
  ...
  let list = [];
  try {
    if (isOnline() && window.sb) list = await fbGetReviews();
